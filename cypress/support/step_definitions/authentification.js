...
When('I register with email {string} and password {string} and firstname {string} and age {string} and I accept the data policy before submitting', (email, password, firstname, age) => {
  const emailValue = (email === 'unique@example.com') ? `${guid()}-fake@example.com` : email;
...
cy.intercept({method: 'POST', url: 'http://localhost:9000/oauth/make_access_token'}, (req) => {
